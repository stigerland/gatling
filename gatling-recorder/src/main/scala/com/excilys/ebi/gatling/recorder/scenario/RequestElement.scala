...
object RequestElement{
	def apply(r:RequestElement, newStatusCode: Int) = {
		new RequestElement(r.request, newStatusCode)
	}
}

class RequestElement(val request: HttpRequest, val statusCode: Int) extends ScenarioElement {
	val method = request.getMethod.toString

	
	val requestBody = if (request.getContent.capacity > 0 && !(request.getHeader(CONTENT_TYPE) == "application/x-www-form-urlencoded")) Some(new String(request.getContent.array)) else None
	...
	val params: List[(String, String)] =
		if (request.getContent.capacity > 0 && request.getHeader(CONTENT_TYPE) == "application/x-www-form-urlencoded") {
