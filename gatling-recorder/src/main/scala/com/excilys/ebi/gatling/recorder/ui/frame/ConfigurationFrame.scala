...
class ConfigurationFrame extends JFrame with Logging {
	...
	private def populateItemsFromConfiguration {
		...
		txtProxyHost.setText(configuration.proxy.host)
		txtProxyPort.setText(configuration.proxy.port.toString)
		txtProxySslPort.setText(configuration.proxy.sslPort.toString)
