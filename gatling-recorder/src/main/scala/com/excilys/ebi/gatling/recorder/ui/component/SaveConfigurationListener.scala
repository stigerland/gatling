...
class SaveConfigurationListener(configurationFrame: ConfigurationFrame) extends ActionListener with Logging {
	...
	def actionPerformed(e: ActionEvent) {
		...
		configuration.proxy.host = configurationFrame.txtProxyHost.getText.trim

		if (!configuration.proxy.host.isEmpty) {
			// Parse outgoing proxy port
			configuration.proxy.port = configurationFrame.txtProxyPort.getText.toInt

			// Parse outgoing ssl proxy port
			configuration.proxy.sslPort = configurationFrame.txtProxySslPort.getText.toInt
		}
