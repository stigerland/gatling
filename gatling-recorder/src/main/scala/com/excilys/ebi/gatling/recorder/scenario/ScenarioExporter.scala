...
object ScenarioExporter extends Logging {
	...
	def saveScenario(startDate: Date, scenarioElements: List[ScenarioElement]) = {
			...
			if(l.isEmpty)
				...
				l.head match{
					case (lastRequestElement: RequestElement, lastStatusCode: Int) =>
						if(lastStatusCode == 302 || lastStatusCode == 301)
							e match{
		...
		// Updates URLs that contain baseUrl and set ids on requests
		var i = 0
		...
		// Aggregate headers
		val headers = filteredElements.map{
		...
		val (newScenarioElements, chains) = getChains(filteredElements)
	...
	private def getChains(scenarioElements: List[ScenarioElement]) : (List[ScenarioElement], List[List[ScenarioElement]]) = {
		...
		if (scenarioElements.size > ScenarioExporter.EVENTS_GROUPING) {
			...
			for (i <- 0 until numberOfSubLists){
				...
				newScenarioElements = new ChainElement :: newScenarioElements
			}
		} else {
			newScenarioElements = scenarioElements
		}
		
		(newScenarioElements, chains)
	}
	
	private def dumpRequestBody(idEvent: Int, content: String, startDate: Date) {
		val dir =
			if (configuration.requestBodiesFolder == null)
				Directory(getOutputFolder / DATE_FORMATTER.format(startDate) + "_" + GATLING_REQUEST_BODIES)
			else
				getFolder("request bodies", configuration.requestBodiesFolder)

		dir.createDirectory()

		var fw: FileWriter = null
		try {
			fw = new FileWriter(File(dir / DATE_FORMATTER.format(startDate) + "_request_" + idEvent + ".txt").jfile)
			fw.write(content)
		} catch {
			case e: IOException =>
	...
	def getOutputFolder = getFolder("output", configuration.outputFolder)

	private def getFolder(folderName: String, folderPath: String) = Directory(folderPath).createDirectory()
}